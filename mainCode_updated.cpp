...
void get_entities(int & wumpus, vector<int> & bats, vector<int> & pits, int &hint){
	...
	hint = rand() % 18+ 3;
	full_rooms.push_back(hint);

	while(entity_amount < 5){
		bool tempvalue = true;
		int temp = (rand() % 18 + 3);
...
int Checklocation(int Playerlocation,vector<vector<int>>Map,int wumpus,int Bat1, int Bat2,int traproom1,int traproom2, int hintRoom){
...
void player_action(int & Playerlocation,vector<vector<int>>Map,int & wumpus,vector<int>bats,vector<int>pits,int & arrowcount,bool normalmode, int &hintKamer){
    ...
    string hintMessage = "beware of room: ";
    bool hint = false;
    int hints = 0;
...
//Gahierverder
			...
			//hint kamer
			else if(inhoud_huidige_kamer == 4){
            ...
            switch(hints){
                case 1: cout << hintMessage << traproom1;
                    break;
                case 2: cout << hintMessage << traproom2;
                    break;
                case 3: cout << hintMessage << bat1;
                    break;
                case 4: cout << hintmessage << bat2;
                    break;
            }
        }
...
void make_map_file (vector<vector<int>> map, int wumpuslocation, vector<int> batslocation, vector<int> pitslocation, int hintRoomLocation) {
    ofstream myfile;
    myfile.open("map.txt");
...
void read_config_file (vector<vector<int>> & map, int & wumpus, vector<int> & bats, vector<int> & pits, int &hint) {
	...
	if (myfile.is_open()){
		while ( getline (myfile, line) ){
			...
			else if (line.find("hint room") != string::npos) {
				while (!ss.eof()) {
					ss >> temp;
					if (stringstream(temp) >> found){
						 hint = found;
...
void printRoundInfo(vector<vector<int>> map, int wumpus, vector<int> bats, vector<int> pits, int hint){
    string answer;
	...
	animatetext("\nThe hint room was: " + to_string(hint));
...
int main(){
	...
	while(!(Nogmaalsspelen=="N") && !(Nogmaalsspelen == "n")){ // hier word het spel gestart
		...
		if(normalmode){
			...
			get_entities(wumpus, Bats, Pits, hint ); //verandert the variables die worden meegegeven
			make_map_file(Map,wumpus,Bats,Pits);
		}
		else{
			read_config_file(Map, wumpus, Bats, Pits, hint);
		}
		player_action(Playerlocation,Map,wumpus,Bats,Pits,hint,arrowcount, normalmode); //Hier word het spel gespeeld, met de map die word bepaald uit de gamemode functie
		printRoundInfo(Map, wumpus, Bats, Pits, hint);
