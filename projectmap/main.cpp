...
void robot_action(int & Playerlocation, vector<vector<int>>Map, int & wumpus, vector<int>Bats, vector<int>pits, int & arrowcount, bool normalmode, vector<int> & rooms_to_avoid, int wumpusgevonden, bool & aiwin){
	...
	while(true){// Hier geven we de speler wat extra informatie over wat er zich in de kamers om hem heen bevint
		...
		if(choice == "M" || choice == "m"){
			...
			while(room_clear = false){
				room_clear = true;
				roomtomoveto = rand() % 3;
				for(unsigned int i3 = 0; i3 < rooms_to_avoid.size();i3++){
					if(Map[Playerlocation][roomtomoveto] == rooms_to_avoid[i3]){
						room_clear = false;
					}
				}
...
int main(){ // deze code bevat onderdelen van iedereen omdat dit de "Hoofdfunctie is"
	...
	while(!(Nogmaalsspelen=="N") && !(Nogmaalsspelen == "n")){ // hier word het spel gestart
		...
		if(robomode == 1){
			...
			robot_action(Playerlocation,Map,wumpus,Bats,Pits,arrowcount,normalmode,rooms_to_avoid,wumpusgevonden,aiwin);
		}
